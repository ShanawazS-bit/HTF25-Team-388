...
const EventGridResales = () => {
    ...
    if (isLoadingEvents) return <p className="message loading-message">Loading resale events...</p>;
    // After events are loaded, if we are fetching other data, show a more specific message
    if ((isLoadingEndTime || isLoadingStartPurchaseTime) && allEvents.length > 0) {
        return <p className="message loading-message">Checking event statuses for resale...</p>;
    }
