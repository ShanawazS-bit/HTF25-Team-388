...
//custom hooks
import FirstEventCard from '../MyTickets/FirstEventCard';
...
const EventGrid = () => {
    ...
    if (isLoadingEvents) return <p className="message loading-message">Loading events...</p>;
    // After events are loaded, if we are fetching end times, show a more specific message
    if (isLoadingEndTime && allEvents.length > 0) return <p className="message loading-message">Checking event statuses...</p>;
